...
RSpec.describe Codebreaker::Game do
  ...
  describe '#check_the_guess' do
    it 'reduces attempts counter by 1' do
      expect { game.check_the_guess('1234') }.to change(game, :attempts).by(-1)
    end

    it 'returns hash' do
      expect(game.check_the_guess('1234').class).to eq(Hash)
    end

    it 'returns hash with 3 keys' do
      expect(game.check_the_guess('1234').keys.size).to eq(3)
    end

    it 'returns integer hash' do
      game.check_the_guess('1234').each_value { |el| expect(el.class).to eq(Integer) }
    end
    ...
    data.each do |d|
      it "returns #{d[2]} with guess #{d[1]} and code #{d[0]} " do
        ...
        expect(game.check_the_guess(d[1]).values).to eq(d[2])
      end
    end
  end
end
